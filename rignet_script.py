...
from os import path
import sys
sys.path.append(path.abspath('./RigNet'))
...
if __name__ == '__main__':
    #root_folder = ""
    #input_folder = "meshes/suriya"

    if(len(sys.argv)<2):
        print("script usage: python quick_start.py <name of model without 'remesh'>")
        sys.exit(0)

    input_model = str(sys.argv[1])
    input_folder = "Results/"+input_model+'/'
    ...
    jointNet.to(device)
    jointNet.eval()
    jointNet_checkpoint = torch.load('Checkpoints/Rignet/gcn_meanshift/model_best.pth.tar', map_location = device)
    ...
    rootNet.to(device)
    rootNet.eval()
    rootNet_checkpoint = torch.load('Checkpoints/Rignet/rootnet/model_best.pth.tar', map_location = device)
    ...
    boneNet.to(device)
    boneNet.eval()
    boneNet_checkpoint = torch.load('Checkpoints/Rignet/bonenet/model_best.pth.tar', map_location = device)
    ...
    skinNet_checkpoint = torch.load('Checkpoints/Rignet/skinnet/model_best.pth.tar', map_location = device)
    ...
    model_id, bandwidth, threshold = input_model, 0.045, 0.75e-5
    #->model_id, bandwidth, threshold = input_model, 0.05, 1e-5
    ...
    print("Saving result")
    if os.path.exists(input_folder+'{:s}_ori.obj'.format(model_id)):
        # here we use original mesh tesselation (without remeshing)
        print("Using original mesh for skin")
